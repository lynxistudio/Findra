...
## 2026-09-23
- Fixed unwanted window popups while in the background: resolved an issue where `WindowAccessor.updateNSView` repeatedly invoked `onResolve(window)` on every SwiftUI state change (scans, timers, file counts), which in turn called `adoptMainWindow` and forcibly activated and ordered the window frontmost while the user was working in other apps. Separated one-time cold launch window presentation from state configuration, guarded `WindowAccessor` to attach only on actual window change, eliminated aggressive `orderFrontRegardless()`, and removed arbitrary `NSApp.activate` calls from background render paths.
- Fixed delayed window appearance and silent crash on launch: eliminated silent `NSInternalInconsistencyException` crash caused by modifying `@Published` properties on a background thread during initialization, guaranteed a minimum 1050x700 content size on first layout to avoid zero-sized (0x0) windows, and added instant frontmost activation so clicking the Dock icon or opening the app immediately pops the window to the front without multiple clicks.
